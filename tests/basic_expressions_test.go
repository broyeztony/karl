...
func TestExpressionKinds(t *testing.T) {
	cases := []struct {
		...
		{
			name:  "as_expression_member_shape",
			input: "decodeJson(raw) as Shapes.HttpResponse",
			assert: func(t *testing.T, expr ast.Expression) {
				ae, ok := expr.(*ast.AsExpression)
				if !ok {
					t.Fatalf("expected AsExpression, got %T", expr)
				...
				if _, ok := ae.Shape.(*ast.MemberExpression); !ok {
					t.Fatalf("expected member shape, got %T", ae.Shape)
				}
			},
		},
		{
		...
		{
			name:  "recover_expression_after_as_member",
			input: `decodeJson(raw) as Shapes.HttpResponse ? { headers: { acceptEncoding: "gzip", }, }`,
			assert: func(t *testing.T, expr ast.Expression) {
				re, ok := expr.(*ast.RecoverExpression)
				if !ok {
					t.Fatalf("expected RecoverExpression, got %T", expr)
				}
				ae, ok := re.Target.(*ast.AsExpression)
				if !ok {
					t.Fatalf("expected AsExpression target, got %T", re.Target)
				}
				if _, ok := ae.Shape.(*ast.MemberExpression); !ok {
