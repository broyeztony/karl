...
// Parse parses a .shape file content into a Shape.
func Parse(content string) (*Shape, error) {
	...
	first := lines[0]
	if first.indent != 0 {
		return nil, lineError(first, "top-level shape must start at indent 0")
	}
	name, typeStr, err := splitDecl(first.text)
	if err != nil {
		return nil, lineError(first, err.Error())
	}
	if !isIdent(name) {
		return nil, lineError(first, "shape name must be a valid identifier")
	}
	rootType, err := parseType(typeStr)
	if err != nil {
		return nil, lineError(first, err.Error())
	}
	shape := &Shape{Name: name, Type: rootType}

	contexts := []context{}
	if obj := ObjectType(rootType); obj != nil {
		contexts = append(contexts, context{indent: first.indent + 4, target: obj})
	}
